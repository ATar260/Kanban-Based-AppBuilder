...
export function generateVersionName(
    ...
    switch (trigger) {
        case 'ticket_done':
            return `${num} ✅ ${ticketTitle || 'Completed'}`.trim();
        case 'ticket_skipped':
            return `${num} ⏭️ ${ticketTitle || 'Skipped'}`.trim();
        case 'ticket_failed':
            return `${num} ❌ ${ticketTitle || 'Failed'}`.trim();
        case 'manual_save':
            return `${num} 💾 Manual save`.trim();
        case 'auto_save':
            return `${num} 🔄 Auto-save`.trim();
        case 'build_start':
            return `${num} 🚀 Build started`.trim();
        case 'build_complete':
            return `${num} 🎉 Build complete`.trim();
        case 'initial':
            return `${num} 📦 Initial`.trim();
        default:
